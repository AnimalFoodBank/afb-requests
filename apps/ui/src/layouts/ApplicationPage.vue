...
onMounted(async () => {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      /**
       * TODO:
       * Basing the is_authenticated value on the response status
       * code is not ideal. The response status code is 200 even
       * if the user is not authenticated. The response data
       * contains the user object if the user is authenticated.
       * It also leads to a sitch where some parts of the UI are
       * rendered before the user object is set; so the global
       * nav is rendered with the auth links but the profie
       * dropdown has the guest links.
       *
       **/
...
/**
 * The navigationItems computed property will be re-computed whenever
 * the user.is_authenticated reactive property changes. In your case,
 * navigationItems will return either profileNav or guestNavigation
 * based on the user.is_authenticated property. So, whenever the
 * authentication status of the user changes (i.e., user.is_authenticated
 * changes), Vue.js will automatically re-compute the navigationItems.
**/
const navigationItems = computed(() => {
  console.log("navigationItems computed(): user.is_authenticated=", user.is_authenticated)
