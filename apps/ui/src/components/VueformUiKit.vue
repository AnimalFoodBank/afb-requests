<template>
  <div>
    <Vueform v-model="sizeForm" sync add-class="mb-10 mx-auto max-w-[180px]">
      <RadiogroupElement name="size" view="tabs" :items="['sm', 'md', 'lg']" />
    </Vueform>

    <Vueform :size="size" :steps-controls="false" :steps="{
      personal: { label: 'Personal details', },
      contact: { label: 'Contact details' },
      social: { label: 'Social' },
    }" @mounted="handleStepsMounted" />

    <Vueform :display-errors="false" :columns="{ label: 12 }" :size="size" :schema="schema" />
  </div>
</template>
